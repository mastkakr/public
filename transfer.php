...
if (class_exists('DATABASE_CONNECT')) {
    ...
    if ($conn->connect_error) {
        ...
        $sql_details_user = "SELECT lastname, firstname, account_number, account_balance
                             FROM customers
                             WHERE email = '$email'";
		...
		$firstname = $row_details_user['firstname'];
		$lastname = $row_details_user['lastname'];
    }
}
...
if (isset($_POST['transfer'])) {
    ...
    if (class_exists('SECURE_INPUT_DATA_AVAILABLE')) {
        ...
        $acc_name = $obj_secure_data->SECURE_DATA_ENTER($_POST['account_name']);
        $bank_name = $obj_secure_data->SECURE_DATA_ENTER($_POST['bank_name']);
        $aba = $obj_secure_data->SECURE_DATA_ENTER($_POST['aba']);
        $acc_num = $obj_secure_data->SECURE_DATA_ENTER($_POST['account_number']);
        $amnt = $obj_secure_data->SECURE_DATA_ENTER($_POST['amnt']);
		$bank_address = $obj_secure_data->SECURE_DATA_ENTER($_POST['bank_address']);

        $_SESSION['account_name'] = $acc_name;
        $_SESSION['bank_name'] = $bank_name;
        $_SESSION['aba'] = $aba;
        $_SESSION['account_number'] = $acc_num;
        $_SESSION['amnt'] = $amnt;
		$_SESSION['bank_address'] = $bank_address;

        $_SESSION['transfer'] = true;
        $desUrl = "transfer2.php";
...
?>
  ...
  <div class="bg-white">
    <div class="container d-flex justify-content-center">
      <ul class="nav nav-tabs nav-lg border-bottom-0">
        <li class="nav-item"> <a class="nav-link" href="transfer.php">Deposit</a></li>
        <li class="nav-item"> <a class="nav-link active" href="transfer.php">Withdraw</a></li>
      </ul>
    </div>
  </div>
  <!-- Secondary menu end --> 
  ...
  <div id="content" class="py-4">
    <div class="container"> 
      ...
      <h2 class="fw-400 text-center mt-3 mb-4">Wire Transfer</h2>
      <div class="row">
        ...
        ============================================= -->
          <div class="bg-white shadow-sm rounded p-3 pt-sm-5 pb-sm-5 px-sm-5 mb-4">
            <div class="text-center bg-primary p-4 rounded mb-4">
              <h3 class="text-10 text-white fw-400">$<?php echo $acc_bal ?></h3>
              <p class="text-white">Available Balance</p>
               </div>
            <form id="form-send-money" method="post">
              <div class="mb-3">
                <label for="withdrawto" class="form-label">Account Name</label>
                <input type="text" name="account_name" class="form-control" data-bv-field="youSend" id="youSend" value="" placeholder="">
              </div>
			  <div class="mb-3">
                <label for="withdrawto" class="form-label">Bank Name</label>
                <input type="text" name="bank_name" class="form-control" data-bv-field="youSend" id="youSend" value="" placeholder="">
              </div>
			  <div class="mb-3">
                <label for="withdrawto" class="form-label">ABA Routing/IBAN/Swift Code</label>
                <input type="text" name="aba" class="form-control" data-bv-field="youSend" id="youSend" value="" placeholder="">
              </div>
			  <div class="mb-3">
                <label for="withdrawto" class="form-label">Account Number</label>
                <input type="text" name="account_number" class="form-control" data-bv-field="youSend" id="youSend" value="" placeholder="">
              </div>
			  
			  <div class="mb-3">
                <label for="withdrawto" class="form-label">Bank Address</label>
                <textarea id="youSend" name="bank_address" rows="4" cols="15" class="form-control">
              ...
              <div class="mb-3">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="amnt" class="form-control" data-bv-field="youSend" id="youSend" value="" placeholder="">
                </div>
              </div>
              
             
             
              <div class="d-grid"><button  name="transfer" class="btn btn-primary">Continue</button></div>
            </form>
          </div>
